	...
	rawFile.onreadystatechange = () => {
		if (rawFile.readyState === 4 && rawFile.status == "200") {
			...
			const renderCardProduct = () => {
				const parent = document.querySelector(".product-list") ? document.querySelector(".product-list") : document.querySelector(".hot-offer-item");
			...
			function renderListItem() {
						...
						if (
							...
							parent
								...
								.addEventListener("click", () => {
									...
									new_list_product[index].wasBuy = parseInt(
										e.target.value
									);
				...
				ItemInCart.forEach((item) => {
					item.addEventListener("click", () => {
						deleteItem(item.parentNode.querySelector("p").innerHTML);
					});
				});
			}
			...
			$(".add-cart").on("click", (e) => {
				...
				localStorage.setItem("item-buy", JSON.stringify(list_product_buy));
				parent.innerHTML = "";
				new renderListItem();
			});
